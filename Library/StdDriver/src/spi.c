...
/**
  ...
  * @note   If u32BusClock >= system clock frequency, SPI peripheral clock source setting in CLKSEL1 register will be set to
  *         select HCLK. The DIVIDER (SPI_DIVIDER[7:0]) will be set to 0.
  * @note   In Slave mode, the SPI peripheral clock rate will be set to equal a half of system clock rate. If the SPI peripheral
  *         clock source is PLL before calling this function, the SPI peripheral clock source setting in CLKSEL1 register will be
  *         changed to select HCLK as the source. The DIVIDER (SPI_DIVIDER[7:0]) will be set to 0 as well.
  */
...
/**
  ...
  * @details This function will calculate the actual SPI bus clock rate according to the settings of SPIn_S and DIVIDER. Only available in Master mode.
  */
uint32_t SPI_GetBusClock(SPI_T *spi)
{
    ...
    /* f_spi = f_spi_clk_src / ((DIVIDER + 1) * 2) */
...
void SPI_ClearIntFlag(SPI_T *spi, uint32_t u32Mask)
{
    if(u32Mask & SPI_UNIT_INT_MASK)
        spi->STATUS = SPI_STATUS_IF_Msk; /* Clear unit transfer interrupt flag */
